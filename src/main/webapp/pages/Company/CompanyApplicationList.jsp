            ...
            <div class="row">
                            ...
                            <div class="select-Categories pb-50">
                                ...
                                <div class="small-section-tittle2">
                                    <h4 class="p-2 pl-3 pb-1 rounded">
                                        <a href="<%=request.getContextPath()%>/company/job/addjob">
                ...
                <div class="col-xl-9 col-lg-9 col-md-8">
                    ...
                    <section class="featured-job-area">
                        <div class="container">
                            <%
                                List<JobApplication> A = (List<JobApplication>) request.getAttribute("AppliedList");

                                if (!A.isEmpty()) {
                                    for (JobApplication a : A) {
                                        if (a.getApproved().equalsIgnoreCase("NO")) continue;
                                        Job j = JobService.getJobById(a.getJid());
                                        Employee e = EmployeeService.getEmployeeById(a.getEid());
                                        List<Recommendation> R = EmployeeService.getRecommendationList(e.getUemail());
                            %>
                            <!-- single-job-content -->
                            <div class="single-job-items mb-30">
                                    ...
                                    <div class="company-img">
                                        <a href="#">
                                            <img class="h-16 w-16 rounded-full" alt=""
                                                 src="<%=request.getContextPath()%>/files/user/<%=e.getID()%>/profile.png"
                                                 onerror=this.src="<%=request.getContextPath()%>/assets/img/user/null/profilepic.jpeg"></a>
                                    </div>
                                ...
                                <div class="items-link items-link2 f-right">
                                    <h3 class="pb-1"><u>Recommended by:</u></h3>

                                    <%
                                        for (Recommendation r : R) {
                                            Employee rec = EmployeeService.getEmployeeById(String.valueOf(r.getRecID()));
                                    %>
                                    <h2><%=rec.getFullname()%>
                                    </h2>
                                </div>
                                ...
                                <div class="items-link items-link2 f-right">
                                    <%if (a.getApproved().equalsIgnoreCase("YES")) {%>
                                    <a class="bg-green-400 border-white text-white hover:bg-red-500">Accepted</a>
                                    <% } else if (a.getApproved().equalsIgnoreCase("No")) {%>
                                    <a class="bg-red-400 border-white text-white hover:bg-red-500">Denied</a>
                                    <% } else if (a.getApproved().equalsIgnoreCase("WAIT")) {
                                        LocalDateTime now = LocalDateTime.now();
                                        DateTimeFormatter formatter = DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm");
                                        String formatDateTime = now.format(formatter);
                                    %>
                                    <form action="<%=request.getContextPath()%>/company/application/update"
                                          method="post">
                                        <input type="text" name="eid" value="<%=e.getID()%>" hidden>
                                        <input type="text" name="jid" value="<%=j.getID()%>" hidden>
                                        <div class="grid gap-x-8 gap-y-4 grid-cols-2">
                                            <button data-toggle="collapse" data-target="#collapse<%=e.getID()%>"
                                                    type="button"
                                                    class=" trans-effect border-2 border-green-400 text-green-400 hover:bg-green-400 hover:text-white py-2 px-4 rounded-full">
                                                Approve
                                            </button>
                                            <button name="approve" value="NO"
                                                    class=" trans-effect border-2 border-red-500 text-red-500 hover:bg-red-500 hover:text-white py-2 px-4 mr-1 rounded-full">
                                                Deny
                                            </button>
                                            <div class="collapse col-span-2" id="collapse<%=e.getID()%>">
                                                <div class="d-flex justify-content-between">
                                                    <div class="d-flex flex-column justify-content-start">
                                                        <label for="datetime<%=e.getID()%>" class="m-0"> Interview
                                                            Time:</label> <br>
                                                        <input type="datetime-local" id="datetime<%=e.getID()%>"
                                                               name="datetime" value="<%=formatDateTime%>"
                                                               min="<%=formatDateTime%>"
                                                               class="border-bottom border-primary">
                                                    </div>
                                                    <button name="approve" value="YES"
                                                            class=" trans-effect border-2 border-green-400 bg-green-400 text-white hover:bg-green-600  py-2 px-4 rounded-full">
                                                        Confirm
                                                    </button>
                                                </div>
                                            </div>
...
<!-- JS here -->
<script>
    deadline = document.getElementById("datetime");
